...
public class World
{
	...
	final List<Invaders> invaders = new ArrayList<Invaders>();
	...
	private void generateInvaders()
    {
		for (int row = 0; row < 4; row++)
        {
            ...
            {
				Invaders invader = new Invaders(-WORLD_MAX_X / 2 + column * 2f,	0, WORLD_MIN_Z + row * 2f);
	...
	private void updateInvaders(float deltaTime)
    {
		...
		for (int i = 0; i < len; i++)
        {
			Invaders invader = invaders.get(i);
			invader.update(deltaTime, speedMultiplier);

			if (invader.state == Invaders.INVADER_ALIVE)
            {
			...
			if (invader.state == Invaders.INVADER_DEAD 	&& invader.stateTime > Invaders.INVADER_EXPLOSION_TIME)
            {
				invaders.remove(i);
				i--;
				len--;
	...
	private void checkShotCollisions()
    {
		...
		for (int i = 0; i < len; i++)
        {
			...
			if (shot.velocity.z < 0)
            {
				...
				for (int j = 0; j < len2; j++)
                {
					Invaders invader = invaders.get(j);
					if (OverlapTester.overlapSpheres(invader.bounds, shot.bounds) && invader.state == Invaders.INVADER_ALIVE)
                    {
						invader.kill();
						listener.explosion();
						score += 10;
