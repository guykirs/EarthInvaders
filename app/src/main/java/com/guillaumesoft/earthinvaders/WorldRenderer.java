...
public class WorldRenderer
{
	...
	public WorldRenderer(GLGraphics glGraphics)
    {
		...
		camera = new LookAtCamera(67, glGraphics.getWidth() / (float) glGraphics.getHeight(), 0.1f, 100);
...
//		ambientLight.enable(gl);
	...
	private void renderInvaders(GL10 gl, List<Invaders> invaders, float deltaTime)
    {
		...
		for (int i = 0; i < len; i++)
        {
			...
			if (invader.state == Invaders.INVADER_DEAD)
            {
				gl.glDisable(GL10.GL_LIGHTING);
